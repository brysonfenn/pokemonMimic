...
void calculate_stats(Pokemon * pok, int level, int level_min, int level_max) {
    ...
    int hp_ev = pokemon_get_ev(pok, IV_HP);
    int atk_ev =pokemon_get_ev(pok, IV_ATTACK);
    int def_ev = pokemon_get_ev(pok, IV_DEFENSE);
    int spd_ev = pokemon_get_ev(pok, IV_SPEED);
    ...
    pok->maxHP = (((2.0 * pok->maxHP + hp_iv + (hp_ev/4)) * level) / 100) + level + 10;
    pok->baseAttack = (((2.0 * pok->baseAttack + atk_iv + (atk_ev/4)) * level) / 100) + 5;
    pok->baseDefense = (((2.0 * pok->baseDefense + def_iv + (def_ev/4)) * level) / 100) + 5;
    pok->baseSpAttack = (((2.0 * pok->baseSpAttack + sp_atk_iv + (atk_ev/4)) * level) / 100) + 5;
    pok->baseSpDefense = (((2.0 * pok->baseSpDefense + sp_def_iv + (def_ev/4)) * level) / 100) + 5;
    pok->baseSpeed = (((2.0 * pok->baseSpeed + spd_iv + (spd_ev/4)) * level) / 100) + 5;
