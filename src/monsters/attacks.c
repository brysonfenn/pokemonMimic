...
int perform_attack(struct Pokemon *perp, int move_num, struct Pokemon *victim, bool enemy) {
    ...
    attack_func_ptr funcp = chosenAttack.side_effect;

    if (victim->currentHP <= 0 && funcp != &self_heal && funcp != &self_inflict_damage && funcp != self_inflict_condition &&
    funcp != increment_self_stat && funcp != increment_self_stat2) {
        return ATTACK_SUCCESS;
    }
